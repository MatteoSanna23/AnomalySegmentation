...
NUM_CLASSES = 20  # 19 Cityscapes + 1 Anomaly (COCO)
...
def get_pixel_scores(pred_logits, pred_masks):
    ...
    MODIFIED: 
    1. Standardizes calculation on first 19 classes for fair comparison.
    2. Extracts the 20th class (Learned Anomaly) for superior detection.
    """
    ...
    # We slice only the first 19 logits (Cityscapes classes)
    logits_19 = pred_logits[:, :, :19]
    
    # CRITICAL: We compute Softmax ONLY on these 19 logits.
    # This forces the probability distribution to sum to 1 over the KNOWN classes,
    # mathematically simulating the behavior of the old ArcFace model without the anomaly class.
    ...
    # --- PART B: LEARNED ANOMALY METRIC (THE NEW POWER) ---
    # We compute Softmax on ALL 20 classes to see the true model confidence
    probs_20 = F.softmax(pred_logits, dim=-1)
    
    # Extract only the 20th channel (Index 19) -> The Anomaly Class
    anomaly_prob = probs_20[:, :, 19:20]
    
    # Create the anomaly heatmap
...
def main():
    ...
    entropy_list = []
    rba_list = []
    learned_list = [] # Nuova lista per la metrica Learned Anomaly
    ood_gts_list = []
    ...
    for path in input_files:  # for each image path
        ...
        with torch.no_grad():
            ...
            # Obtain pixel-wise maps (NOW RETURNS 3 MAPS)
            sem_probs_std, pixel_logits_std, learned_map = get_pixel_scores(pred_logits, pred_masks)

            # Convert to numpy (remove batch dim) because we have batch size 1
            sem_probs_np = sem_probs_std.squeeze(0).cpu().numpy()  # (19, 512, 1024)
            pixel_logits_np = pixel_logits_std.squeeze(0).cpu().numpy()  # (19, 512, 1024)
            learned_np = learned_map.squeeze(0).cpu().numpy() # (1, 512, 1024)

            # -- COMPUTATION 1 : MSP (Standardized) --
            msp_score = 1 - np.max(
                sem_probs_np, axis=0
            )  # 1 - max(Known Class Probabilities)

            # -- COMPUTATION 2 : MaxLogit (Standardized) --
            maxLogit_score = -np.max(
                pixel_logits_np, axis=0
            )  # - max(Known Class Logits)

            # -- COMPUTATION 3 : Entropy (Standardized) --
            entropy_score = -np.sum(sem_probs_np * np.log(sem_probs_np + 1e-8), axis=0)

            # -- COMPUTATION 4 : RbA (Standardized) --
            # Nota: con la rinormalizzazione a 19 classi, la somma sarà ~1, quindi RbA sarà basso.
            # Ma lo manteniamo per compatibilità.
            rba_score = 1 - np.sum(
                sem_probs_np, axis=0
            )

            # -- COMPUTATION 5 : Learned Anomaly (Classe 20) --
